...
T = TypeVar('T', bound='CustomCollectionElement')


class CustomCollectionElementPredefinedFilter(str, Enum):
    disabled = "disabled"
    enabled = "enabled"
...
class CustomCollectionElement(JsonSerializableDict):
    # https://developer.xurrent.com/v1/custom_collection_elements/
    __resourceUrl__ = 'custom_collection_elements'
    ...
    def __init__(self,
        ...
        from .custom_collections import CustomCollection
        self.custom_collection = (custom_collection if isinstance(custom_collection, CustomCollection)
                                  else CustomCollection.from_data(connection_object, custom_collection)
                                  if isinstance(custom_collection, dict) else custom_collection)

        for key, value in kwargs.items():
            setattr(self, key, value)

    def __str__(self) -> str:
        return (f"CustomCollectionElement(id={self.id}, name={self.name}, "
                f"reference={self.reference}, disabled={self.disabled})")
    ...
    @classmethod
    def get_custom_collection_elements(cls, connection_object: XurrentApiHelper,
                                       predefinedFilter: CustomCollectionElementPredefinedFilter = None,
    ...
    def update(self, data: dict) -> T:
        ...
        return CustomCollectionElement.from_data(self._connection_object, response)
